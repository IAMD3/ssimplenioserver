...
public class HttpUtilTest {
    ...
    public static void main(String[] args) throws UnsupportedEncodingException {
        ...
        Request request = HttpUtil.parseHttpRequest(xBuffer);
       ...
       //模拟第一次nio下读到不完整的业务数据包 -> 无法反序列化成一个http报文
        ...
        Request request_non = HttpUtil.parseHttpRequest(xBuffer);
        //cannot parse an incomplete request
        Assert.checkNull(request_non);
        //模拟第二次nio下读到上次不完整的业务数据包其他部分+一个完整的业务数据包场景...承接第一次读到的数据 ->
        xBuffer.cache(bytes_part_2);
        //now we can decode two consequent HTTP Requests
        Request complete_request_1 = HttpUtil.parseHttpRequest(xBuffer);
        Request complete_request_2 = HttpUtil.parseHttpRequest(xBuffer);
